...
public class TransactionsService {
    ...
    public TransactionsResponse.TransactionResponseItem retrieve(String reference){
        ...
        if(transactionDTO.getCredit() != null){
            transactionResponseItem.setAmount(transactionDTO.getCredit());
        }else {
            transactionResponseItem.setAmount(transactionDTO.getDebit().negate());
        }
        return transactionResponseItem;
    }

    public Page<TransactionsResponse.TransactionResponseItem> retrieve(int pageNumber, int pageSize, String walletId){
        Pageable pageable = new DefaultPageRequest(pageNumber, pageSize);
        return transactionDAS.retrieve(walletId, pageable);
    }
}